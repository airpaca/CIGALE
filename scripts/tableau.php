...
// // Si conso alors conso à null et la conso apparait dans le champ val
// $sql = "
// select 
    // an, 
    // " . $nom_entite . "  as nom_entite,  
    // " . $nom_secten1 . " ,  
    // " . $cat_energie . " , 
    // case when nom_abrege_polluant = 'conso' then null else round(sum(val)::numeric, 1) end as conso, -- round(sum(val)::numeric, 1) as conso, 
    // -- case when id_polluant = 131 then -999 else round(sum(val)::numeric, 1) end as conso, 
    // nom_abrege_polluant, 
    // round(sum(val)::numeric, 1) as val, 
    // lib_unite
